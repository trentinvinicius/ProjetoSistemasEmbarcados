/**
\mainpage Luximetro
* \author <br>Daniel Caye e Vinicius Trentin<br>
* \date <br>28 de Junho de 2017
* \brief Trabalho final da disciplina de Sistemas Embarcados <br><br>
*  Placa utilizada: SAM D21 <br><br>
*  O trabalho consiste em realizar a leitura de um sensor de luminosidade (LDR) e baseado nisso modificar a intensidade do brilho de um led. <br><br>
*  Tanto a intensidade em percentual quanto o intervalo de tempo entre as variacoes do nivel da intensidade do led sao armazenados na memoria EEPROM.<br><br>
*/
#include <asf.h>
#include <stdio.h>
#include <string.h>
...
/// Endereco da instacia criada para a USART
int32_t instancia_usart;   
/// Buffer que contem os dados recebidos pela USART, aceita 5 caracteres
...
/** Ponteiro de funcao utilizado para chamar as funcoes referentes a maquina de estados */
typedef void (*Action)(unsigned char data);  

/**
* \brief Estrutura que contem os elementos referentes a maquina de estados
*/
struct StateMachine {
...
/**
* \fn tcc_callback_timer(struct tcc_module *const module_inst)
* \param[in] *const module_inst Estrutura que contem a instancia do modulo tcc.
...
* \param[in] *const usart_module Estrutura que contem a instancia do modulo usart.
